...
| `is_exempt` | boolean | האם העובד פטור ממשמרות (ברירת מחדל: `false`). עובד פטור מוצג עם אזהרה אדומה ברשימת השיבוץ, שיבוץ כפוי עדיין אפשרי |
| `exemption_reason` | string? | סיבת הפטור — **חובה** כשהעובד פטור. טקסט חופשי (לדוגמה: "בהשלמה", "עבר יחידה", "פטור רפואי") |
| `receives_shift_allocation` | boolean | האם מקבלים עבורו הקצאת משמרת למרות הפטור |
| `standing_constraints` | string? | אילוצים קבועים שחוזרים בכל רבעון (כגון ימי לימודים), טקסט חופשי |
| `notes` | string? | הערות על העובד — מקביל לעמודת הערות בקובץ העובדים. כולל אילוצי רבעון נוכחי ומידע רלוונטי אחר. מיובא מטפסי הגשת אילוצים. גלוי גם במסך השיבוץ |
| `release_date` | date? | תאריך תום שירות (תת"ש) — תאריך סיום החוזה. `null` = לא ידוע / לא רלוונטי |
| `created_at` | datetime | |
| `updated_at` | datetime | |
